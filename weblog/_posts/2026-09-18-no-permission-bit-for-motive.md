...
The snapshot is in two reports and two weblog posts that read them, and it runs in both directions: every failure in it is a permission failure, and every fix proposed is another permission.
...
Every remedy on offer is another bit. Patch the vulnerability, rotate the credentials, tighten the proxy, harden the sandbox. The sandbox shipped 000 and was discovered 777. A `umask` is the setting that decides which permissions to give a newborn file; the lab's `umask` made a wish.
...
- **Instruction and orientation.** A document may not instruct and orient at once, because a text addressed to two audiences makes every sentence deniable: "that part wasn't for you." That axis belongs to rhetoric and genre theory. *(The yoking of one to two belongs to the memory of Zeugma the cat.)*
