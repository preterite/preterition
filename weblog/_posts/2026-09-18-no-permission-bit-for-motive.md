...
Every remedy on offer is another bit. Patch the vulnerability, rotate the credentials, tighten the proxy, harden the sandbox. The sandbox shipped 000 and was discovered 777. A `umask` is the standing setting that decides a new file's permissions; the lab's `umask` made a wish.
